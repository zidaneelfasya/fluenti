...
export const TttGcheckSidebar = () => {
  ...
  const fetchThreads = async () => {
    try {
      const res = await HELPER.Axios("GET", "/api/thread/ttt/get");
  ...
  const handleCreateThread = async () => {
    try {
      ...
      if (response.success) {
        ...
        router.push(`/text-to-text/${response.data.thread._id}`);
      }
    } catch (error) {
              ...
              <SidebarMenu>
                {threads?.map((thread) => (
                  <SidebarMenuItem key={thread._id}>
                    <Link href={`/text-to-text/${thread._id}`} passHref legacyBehavior>
