...
export const VtvGcheckSidebar = () => {
  ...
  const fetchThreads = async () => {
    try {
      const res = await HELPER.Axios("GET", "/api/thread/vtv-gcheck/get");
  ...
  const handleCreateThread = async () => {
    try {
      ...
      if (response.success) {
        ...
        router.push(`/vtv-gcheck/${response.data.thread._id}`);
      }
    } catch (error) {
              ...
              <SidebarMenu>
                {threads?.map((thread) => (
                  <SidebarMenuItem key={thread._id}>
                    <Link href={`/vtv-gcheck/${thread._id}`} passHref legacyBehavior>
